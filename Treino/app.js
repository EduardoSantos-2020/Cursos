...
const getoddNumber=randomNumbers =>randomNumbers%2===1

const oddNumber=randomNumbers.filter(getoddNumber)
...
const countnumbersLessThans501 =(accumulator,crazyNumbers) => 
 crazyNumbers < 501 ? ++accumulator :accumulator //forma simplificada
  // if(crazyNumbers < 501){
 ...
 const numbersLessThans501=crazyNumbers.reduce(countnumbersLessThans501,0 )


console.log(numbersLessThans501);
/*
  03
...
const numberQ=numbers.map(numbers=>numbers **2)
...
const films=tarantinoMovies.filter(({release})=>release < 2000)
console.log(films);
...
const namestvshow=tvShows.map(({name}) => name)
...
const gamescard = cart.reduce((accumulator,{name}) => 
`${accumulator} - ${name}\n` ,'')
